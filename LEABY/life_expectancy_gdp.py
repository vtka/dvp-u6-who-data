...
# Update 'United States of America' to USA later for better view
f, ax1 = plt.subplots(figsize=(25, 25)) 
ax1 = sns.barplot(data=df, x='Country', y='GDP') 
plt.xticks(rotation=90, fontsize=15)
ax1.set_title('Gross Domestic Product of Different Countries', fontsize=20)
ax1.set_ylabel('Gross Domestic Product', fontsize=20)
...
# Set style
f, ax2 = plt.subplots(figsize=(25, 25))
sns.set_style('darkgrid')
ax2 = sns.barplot(data=df, x='Country', y='LEABY') 
plt.xticks(rotation=90, fontsize=15 )
ax2.set_title('Life expectancy at birth (years) in Different Countries', fontsize=20)
ax2.set_ylabel('LEABY', fontsize=20)
...
# There is a big disparity between two graphs. GDP data shows drastic difference between countries, however, most of them have insignificant difference in LEABY, except for Zimbabwe. 
...
ax3 = sns.violinplot(data=df, x='Country', y='LEABY')
ax3.set_title('Data Distribution of Life expectancy at birth in Different Countries', fontsize=20)
ax3.set_ylabel('LEABY', fontsize=10)
...
#  Zimbabwe's life expectancy has changed the most. It has the highest distribution of data.
...
ax.set_title('GDP change over time by country', fontsize=20)
plt.savefig('ax.png')
plt.show()
...
ax.set_title('LEABY change over time by country', fontsize=20)
plt.savefig('ax4.png')
plt.show()
...
# 1. The most drastic change is shown by Zimbabwe in LEABY. However Zimbabwe showed almost no change for its GDP. It is because Zimbabwian economy was affected by hyper inflation and currency depreciation rate; China and USA, however, showed amazing growth in GDP; USA had high LEABY before this GDP leap, and showed almost no change. China had a significant increase (5 years) in LEABY over the period of 2000-2015.
# 2. Period of 2005 - 2015 shows the biggest change.
# 3. The least change in GDP is shown by Zimbabwe, Chilie and Mexico.
# 4. The list of countries represented in data can be compared economically.
# 5. GDP is only one of many parameters that affect LEABY. The higher LEABY, the less effect GDP has on it.
# 6. As it was mentioned before, Zimbabwian economy was affected by hyper inflation and currency depreciation rate. Due to the fact that GDP is in $USD, it seems like Zimbabwe's growth was almost 0. Which is wrong, because GDP doesn't reflect differences in the cost of living and the inflation rates of the countries. China is a great example to show correlation between LEABY and GDP, because according to Investopedia.com - Chinese Yuan showed a very little change in it's currency value. And according to Trading Economics, inflation rate was less than around 2.5 percent and sometimes inflation was replaced by deflation. Line chart would show a better picture for such data.
# 
# 
...
# 1. China moves the most along X-axis.
# 2. Zimbabwe moves the most along Y-axis.
# 3. It's not surprising, because GDP doesn't reflect differences in the cost of living and the inflation rates of the countries.
# 4. Scatter plots are hard to read, because you should always look at plot legend in order to follow the graph.
...
# 2. 2005 - 2015
# 3. Mexico
# 4. For the most countries, LEABY was at high level and had insignificant increase over period of 2000-2015. Zimbabwe's LEABY was very low (and still is). The higher LEABY, the harder it is to increase it. Perhaps it's because there are other factors must be taken into consideration for such increase. At some point it only requires to open additional hospitals, increase access to water and food and it will boost LEABY. But for such countries as Germany and USA, to increase it even by few points will require additional extras.
...
# 1. USA has the highest GDP and Zimbabwe has the lowest GDP.

# Which countries have the highest and lowest life expectancy?

# 2. Germany has the highest LEABY, Zimbabwe - the lowest LEABY.
...
# Cheap labor brought corporations from abroad to China. This led to huge increase in production output, which affected GDP. 
